...
iframe.src = "../sandbox.html?url=" + query_fixed;
...
function proxy(iframe){
    setInterval(function(){
        ...
        for (let i = 0; i < html_split.length; i++) {
            if (!html_split[i].startsWith("sandbox.html?url=")) {
                html_split[i] = "../sandbox.html?url=" + html_split[i];
                changed = 1;
            }
        }
